...
export default function Projects() {
  ...
  const [detailView, setDetailView] = useState("folder");
  ...
  const handleSelectClient = (client, mode = "folder") => {
  ...
  if (selectedClient) {
    return (
      <div className="max-w-6xl mx-auto">
        ...
        {detailView === "folder" ? (
          <FolderView client={selectedClient} subProjects={subProjects} tasks={tasks} onBack={handleBack} isAdmin={isAdmin} />
        ) : (
          <TaskList client={selectedClient} tasks={tasks} onBack={handleBack} isAdmin={isAdmin} />
        )}
      </div>
    );
  }
      ...
      {filtered.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-1">
          {filtered.map((client) => {
            ...
            const percent = clientTasks.length ? Math.round((app / clientTasks.length) * 100) : 0;
...
function ProjectFolderRow({ client, subProjects, tasks, percent, onOpen, onEdit, onDelete }) {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 py-2 px-3 rounded-lg hover:bg-gray-50 cursor-pointer group" onClick={() => setOpen(!open)}>
        ...
        {open ? <FolderOpen className="w-5 h-5 flex-shrink-0" style={{ color: client.color || "#3B82F6" }} /> : <Folder className="w-5 h-5 flex-shrink-0" style={{ color: client.color || "#3B82F6" }} />}
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
          ...
          {onEdit && <button onClick={() => onEdit(client)} className="p-1 text-gray-400 hover:text-gray-700 rounded"><Pencil className="w-3.5 h-3.5" /></button>}
          {onDelete && <button onClick={() => onDelete(client)} className="p-1 text-gray-400 hover:text-red-500 rounded"><Trash2 className="w-3.5 h-3.5" /></button>}
        </div>
      </div>
            ...
            subProjects.map((sub) => {
              ...
              return (
                <div key={sub.id} className="ml-2">
                  <div className="flex items-center gap-2 py-1.5 px-2 rounded-lg hover:bg-gray-50 cursor-default">
